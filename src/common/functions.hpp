...
inline auto make_function(Return (*fn)(Args...))
...
inline auto make_function(Return (Class::*fn)(Args...))
    -> std::function<Return(Args...)> {
  return {fn};
}
...
template <typename... Args, typename Return, typename Class>
inline auto make_function(Return (Class::*fn)(Args...) const)
    -> std::function<Return(Args...)> {
  return {fn};
}
...
inline auto make_function(Fn fn) -> std::function<decltype(
...
inline auto make_function(Fn fn) -> decltype(make_function(&Fn::operator())) {
  return {std::move(fn)};
}
