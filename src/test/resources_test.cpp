...
struct B {
  auto operator()(ResourceDeps& deps, int x) {
    return format("B%1%", x);
  }
};

struct C {
  std::string operator()(ResourceDeps& deps, int x) {
    if (x > 0) {
      return format("C%1%(%2%,%3%)", x, deps.get<B>(x), deps.get<C>(x - 1));
    } else {
      return "_";
    }
  }
...
struct D {
  auto operator()(ResourceDeps& deps, const std::string& s, int t) {
    return format("D%1%%2%", s, t);
  }
};

struct E {
  auto operator()(ResourceDeps& deps, const int x, const int y) {
    return format("E%1%%2%", x, y);
  }
};

struct F {
  auto operator()(ResourceDeps& deps, const std::string& s) {
    return format("F%1%", s);
  }
};

auto not_F(ResourceDeps& deps, const std::string& s) {
...
struct H {
  std::string operator()(ResourceDeps& deps, int key) {
...
struct stress_test {
  int operator()(ResourceDeps& deps, int n) {
    if (n == 0) {
      return 0;
    } else if (n == 1) {
      return 1;
...
TEST_CASE("Test overrides", "[resources]") {
  auto not_C = "not_C";
  auto not_D = [](ResourceDeps& deps, const std::string& s, int t) {
    return format("not_D%1%%2%", s, t);
  };

  auto resources =
      ResourcesBuilder()
          .withOverride<A>([](ResourceDeps& deps) { return "not_A"; })
          .withOverride<C>([&](ResourceDeps& deps, int x) {
...
TEST_CASE("Test seed resources", "[resources]") {
  auto g_func = [](ResourceDeps&) -> std::string { return "good2"; };
...
TEST_CASE("Test updates", "[resources]") {
  ...
  resources.invalidate<update_4>();
  REQUIRE(1 == resources.get<update_4>());
  REQUIRE(1 == resources.get<update_4>());
  resources.invalidate<update_3>();
  ...
  resources.invalidate<update_3>();
  resources.invalidate<update_3>();
  REQUIRE(2 == resources.get<update_3>());
  REQUIRE(3 == resources.get<update_4>());
  resources.invalidate<update_1>();
...
TEST_CASE("Test asynchronous resources", "[resources]") {
  AsyncResources resources(
      std::make_shared<Resources>(), std::make_shared<QueueExecutor>(10));
  ...
  resources.invalidate<async_4>().get();
  REQUIRE(1 == resources.get<async_4>().get());
  REQUIRE(1 == resources.get<async_4>().get());
  resources.invalidate<async_3>().get();
  ...
  resources.invalidate<async_3>().get();
  resources.invalidate<async_3>().get();
  REQUIRE(2 == resources.get<async_3>().get());
  REQUIRE(3 == resources.get<async_4>().get());
  resources.invalidate<async_1>().get();
  ...
  resources.invalidate<async_2>().get();
  resources.invalidate<async_3>().get();
  resources.invalidate<async_4>().get();
  REQUIRE(1 == resources.get<async_1>().get());
...
TEST_CASE("Stress test asynchronous resources", "[resources]") {
  ...
  for (int i = 0; i < 10000; i += 1) {
    futures.push_back(resources.invalidate<stress_test>(std::rand() % 50));
  }
