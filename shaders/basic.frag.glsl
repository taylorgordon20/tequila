...
vec3 getAmbientComponent(vec3 normal, vec3 light, vec3 half) {
  return light_ambient * mat_ambient;
}

vec3 getDiffuseComponent(vec3 normal, vec3 light, vec3 half) {
  return max(0.0, dot(normal, light)) * (light_diffuse * mat_diffuse);
}

vec3 getSpecularComponent(vec3 normal, vec3 light, vec3 half) {
  vec3 spec = light_specular * mat_specular;
  return pow(max(0.0, dot(normal, half)), mat_shininess) * spec;
}
...
void main() {
  ...
  vec3 half = normalize(0.5 * (_eye + _light));

  // Compute lighting components.
  vec3 A = getAmbientComponent(normal, light, half);
  vec3 D = getDiffuseComponent(normal, light, half);
  vec3 S = getSpecularComponent(normal, light, half);
